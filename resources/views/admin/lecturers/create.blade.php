    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900 dark:text-gray-100">
            ...
            <form method="POST" action="{{ route('lecturers.store') }}" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <div>
                            ...
                            <input id="lecturer_code" class="block mt-1 w-full rounded-md shadow-sm" type="text" name="lecturer_code" value="{{ old('lecturer_code') }}" required autofocus />
                        </div>
                        <div class="mt-4">
                            ...
                            <input id="full_name" class="block mt-1 w-full rounded-md shadow-sm" type="text" name="full_name" value="{{ old('full_name') }}" required />
                        </div>
                        <div class="mt-4">
                            ...
                            <input id="date_of_birth" class="block mt-1 w-full rounded-md shadow-sm" type="date" name="date_of_birth" value="{{ old('date_of_birth') }}" required />
                        </div>
                        <div class="mt-4">
                        ...
                        <div class="mt-4">
                            ...
                            <input id="email" class="block mt-1 w-full rounded-md shadow-sm" type="email" name="email" value="{{ old('email') }}" required />
                        </div>
                        <div class="mt-4">
                            <label for="phone_number" class="block font-medium text-sm text-gray-700 dark:text-gray-300">{{ __('Số điện thoại') }}</label>
                            <input id="phone_number" class="block mt-1 w-full rounded-md shadow-sm" type="text" name="phone_number" value="{{ old('phone_number') }}" />
                        </div>
                    </div>
                    ...
                    <div>
                        <div>
                            ...
                            <textarea id="address" name="address" rows="3" class="block mt-1 w-full rounded-md shadow-sm">{{ old('address') }}</textarea>
                        </div>
                        <div class="mt-4">
                        ...
                        <div class="mt-4">
                            ...
                            <select id="academic_level" name="academic_level" required class="block mt-1 w-full rounded-md shadow-sm">
                                ...
                                <option value="Cử nhân" {{ old('academic_level') == 'Cử nhân' ? 'selected' : '' }}>Cử nhân</option>
                                <option value="Thạc sĩ" {{ old('academic_level') == 'Thạc sĩ' ? 'selected' : '' }}>Thạc sĩ</option>
                                <option value="Tiến sĩ" {{ old('academic_level') == 'Tiến sĩ' ? 'selected' : '' }}>Tiến sĩ</option>
                                <option value="Phó Giáo sư" {{ old('academic_level') == 'Phó Giáo sư' ? 'selected' : '' }}>Phó Giáo sư</option>
                                <option value="Giáo sư" {{ old('academic_level') == 'Giáo sư' ? 'selected' : '' }}>Giáo sư</option>
                            </select>
                        </div>
                        <div class="mt-4">
                            <label for="position" class="block font-medium text-sm text-gray-700 dark:text-gray-300">{{ __('Chức vụ') }}</label>
                            <input id="position" class="block mt-1 w-full rounded-md shadow-sm" type="text" name="position" value="{{ old('position') }}" />
                        </div>
                        <div class="mt-4">
                ...
                {{-- THÊM MỤC NHẬP HỌC VỊ/HỌC HÀM (MỘT BẢN GHI) --}}
              <h3 class="text-lg font-semibold mt-8 mb-4 border-b pb-2">Học vị/Học hàm (Đầu tiên)</h3>
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    <div>
        {{-- THAY ĐỔI INPUT NÀY --}}
        ...
        <select id="degree_type_id_0" name="academic_degrees[0][degree_type_id]"
            ...
            @foreach ($degreeTypes as $type)
                <option value="{{ $type->id }}" {{ old('academic_degrees.0.degree_type_id') == $type->id ? 'selected' : '' }}>
    ...
    <div>
        ...
        <input id="degree_specialization_0" type="text" name="academic_degrees[0][specialization]" value="{{ old('academic_degrees.0.specialization') }}" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600">
    </div>
    <div>
        <label for="degree_institution_0" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Nơi cấp</label>
        <input id="degree_institution_0" type="text" name="academic_degrees[0][issuing_institution]" value="{{ old('academic_degrees.0.issuing_institution') }}" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600">
    </div>
    <div>
        <label for="degree_date_issued_0" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Ngày cấp</label>
        <input id="degree_date_issued_0" type="date" name="academic_degrees[0][date_issued]" value="{{ old('academic_degrees.0.date_issued') }}" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600">
    </div>
    <div class="md:col-span-2">
        <label for="degree_notes_0" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Ghi chú (Học vị)</label>
        <textarea id="degree_notes_0" name="academic_degrees[0][notes]" rows="2" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600">{{ old('academic_degrees.0.notes') }}</textarea>
    </div>
</div>

                {{-- THÊM MỤC NHẬP QUÁ TRÌNH CÔNG TÁC (MỘT BẢN GHI) --}}
                <h3 class="text-lg font-semibold mt-8 mb-4 border-b pb-2">Quá trình công tác (Đầu tiên)</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="work_organization" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Tên đơn vị/tổ chức</label>
                        <input id="work_organization" type="text" name="work_histories[0][organization_name]" value="{{ old('work_histories.0.organization_name') }}" class="block mt-1 w-full rounded-md shadow-sm">
                    </div>
                    <div>
                        <label for="work_position" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Chức vụ đảm nhiệm</label>
                        <input id="work_position" type="text" name="work_histories[0][position_held]" value="{{ old('work_histories.0.position_held') }}" class="block mt-1 w-full rounded-md shadow-sm">
                    </div>
                    <div>
                        <label for="work_start_date" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Ngày bắt đầu</label>
                        <input id="work_start_date" type="date" name="work_histories[0][start_date]" value="{{ old('work_histories.0.start_date') }}" class="block mt-1 w-full rounded-md shadow-sm">
                    </div>
                    <div>
                        ...
                        <input id="work_end_date" type="date" name="work_histories[0][end_date]" value="{{ old('work_histories.0.end_date') }}" class="block mt-1 w-full rounded-md shadow-sm">
                    </div>
                    <div class="md:col-span-2">
                        ...
                        <textarea id="work_courses_taught" name="work_histories[0][courses_taught]" rows="2" class="block mt-1 w-full rounded-md shadow-sm">{{ old('work_histories.0.courses_taught') }}</textarea>
                    </div>
                    <div class="md:col-span-2">
                        <label for="work_description" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Mô tả công việc</label>
                        <textarea id="work_description" name="work_histories[0][description]" rows="2" class="block mt-1 w-full rounded-md shadow-sm">{{ old('work_histories.0.description') }}</textarea>
                    </div>
                </div>
